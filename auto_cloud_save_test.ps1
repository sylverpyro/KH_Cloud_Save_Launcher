...
Function _cloud_sync_file($save_file){

    Write-Output "Got file: $save_file"
    $folder_name = $save_file.TrimEnd(".png")
    Write-Output "Basename: $folder_name"
    if ( -not(Test-Path -Path $cloud_save_dir/$folder_name -PathType Container)) {
        Write-Output "Did not detect '$cloud_save_dir/$folder_name' want to create"
    }
...
Function _cloud_sync {
    ...
    if ( $newest_cloud_ts -gt $local_save_ts ) {
        ...
        Write-Output "  Would: mv $local_save_dir/$KHFM_save_file $local_save_dir/$save_backups/KHFM-$local_save_ts.png"
    ...
    elseif ( $local_save_ts -gt $newest_cloud_ts ) {
        ...
        Write-Output "  Would: cp $local_save_dir/$KHFM_save_file $cloud_save_dir/KHFM-$local_save_ts.png"
    }
...
foreach ($save in @($KHFM_save_file, $KHCOM_save_file)) {
    #Write-Output "Got save: $save"
    _cloud_sync_file $save
}
